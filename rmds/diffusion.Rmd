...
```{r show data,fig.width = 8, fig.height = 8}
...
```{r fit model, eval = TRUE,fig.width = 8, fig.height = 8}
...
mesh <- inla.mesh.2d(boundary = bdry, max.edge = c(4, 7)/300, cutoff = 4/300) ## WARNING MESH FAR TOO COARSE. FOR ILLUSTRATION ONLY
...
## the model fitted here ignores country levelrandom effects to speed up compuation time
...
fit <- geo.fit(mesh = mesh, locs = locs, response = response, 
               ...
               temp = t.idx, verbose = FALSE, family = "poisson") 
## WARNING NOT A GOOD FIT DUE TO COARSENESS OF MESH. FOR ILLUSTRATION ONLY
...
surf <- list();xmean <- list()
xsd <- list();bdq975 <- list()
bdq25 <- list();bdq975surf <- list()
bdq25surf<-list()
for (j in 1:k){
    xmean[[j]]<-fields[[1]][[j]]
    xsd[[j]]<-sdfields[[1]][[j]]
    surf[[j]]<-poisson(link  = 'log')$linkinv(fit$summary.fix[2,1]*covariates$population + fit$summary.fix[3,1]*covariates$time.to.city + fit$summary.fix[4,1]*covariates$luminosity + fields[[1]][[j]] + fit$summary.fix[1,1])
    bdq975[[j]]<-xmean[[j]]+1.96*xsd[[j]]
    bdq25[[j]]<-xmean[[j]]-1.96*xsd[[j]]
    bdq975surf[[j]]<-poisson(link  = 'log')$linkinv(fit$summary.fix[2,1]*covariates$population + fit$summary.fix[3,1]*covariates$time.to.city + fit$summary.fix[4,1]*covariates$luminosity+ bdq975[[j]]+fit$summary.fix[1,1])
    bdq25surf[[j]]<-poisson(link  = 'log')$linkinv(fit$summary.fix[2,1]*covariates$population +
                                                fit$summary.fix[3,1]*covariates$time.to.city + fit$summary.fix[4,1]*covariates$luminosity+ bdq25[[j]]+fit$summary.fix[1,1])
}
```

```{r}
min <- min(unlist(lapply(surf,min,na.rm  = TRUE)));min
max <- max(unlist(lapply(surf,max,na.rm  = TRUE)));max
logmin <- log(quantile(response, probs = 0.95));logmin
logmax <- log(max);logmax
...
hotrpoly<-list()
for(j in 1:k){
  hotrpoly[[j]] <- rasterToPolygons(hotr[[j]], fun  = NULL, n  = 16, na.rm  = TRUE, digits  = 5, dissolve  = TRUE)
  hotrpoly[[j]] <- unionSpatialPolygons(hotrpoly[[j]], rep(1, length(hotrpoly[[j]])))## put adjacent polygons together
...
for(j in 1:length(hotrpoly)){
  ...
  areas[[j]] <-  sapply(slot(hotrpoly[[j]], "polygons"), function(x) sapply(slot(x,"Polygons"), slot, "area"))
  maxareas[[j]] <- sapply(areas[[j]], max)
  ## in case of possible divided polygons keep the one with highest area (is required for following steps)
...
for(j in 1:k){
  ...
  hotbuff[[j]] <- gBuffer(hotbuff[[j]], byid  = FALSE, id  = NULL, width  = 0.4545455, quadsegs  = 1,capStyle  = "SQUARE")
...
for(j in 1:k){
  ...
  diffgrid[[j]]<- diffgrid[[j]][,!(names(diffgrid[[j]]) %in% drops)]## remove unuseful variable
}
## diffusion
surflowdf<-list()
surflowr<-list()
...
rm(extract) 
meandiff <- list();highdiff <- list()
lowdiff <- list();escpos <- list()
escneg <- list();escdf <- list()
##################OPTIONAL threshold#####################
thres<-0
##################OPTIONAL threshold#####################
for(j in 1:(k-1)){
meandiff[[j]] <- unlist(extract(surfmeanr[[j+1]],diffgrid[[j]],fun  = mean,na.rm  = T))-unlist(extract(surfmeanr[[j]],diffgrid[[j]],fun  = mean,na.rm  = T))
highdiff[[j]] <- unlist(extract(surfhighr[[j+1]],diffgrid[[j]],fun  = mean,na.rm  = T))-unlist(extract(surfhighr[[j]],diffgrid[[j]],fun  = mean,na.rm  = T))
lowdiff[[j]] <- unlist(extract(surflowr[[j+1]],diffgrid[[j]],fun  = mean,na.rm  = T))-unlist(extract(surflowr[[j]],diffgrid[[j]],fun  = mean,na.rm  = T))
  escpos[[j]] <- ifelse(meandiff[[j]]>thres & highdiff[[j]]>thres & lowdiff[[j]]>thres,1,0)
  escneg[[j]] <- ifelse(meandiff[[j]]< -1*thres & highdiff[[j]]< -1*thres & lowdiff[[j]]< -1*thres,1,0)
}
for(j in 1:(k-1)){
...
diffcrop<-list()
 for(j in 1:(k-1)){diffcrop[[j]] <- crop(diffgrid[[j]], extent(40, 75, 25, 40))}
## plot
h <- 1200
w <- 1.2*h
```
...
```{r plot hot,fig.width = 10, fig.height = 10}
...
plot(diffcrop[[1]],col  = ifelse(diffcrop[[1]]$escpos>0,coldif,ifelse(diffcrop[[1]]$escneg>0,coldis,colnull)),
     border  = "NA",main  = "",xlab  = "",ylab  = "",axes  = F,xlim  = c(60,80),ylim  = c(20,40))
plot(hotrpoly[[1]], density  = 10, angle  = 45, col  = colhot,border  = "black",add  = TRUE) 
...
box()
axis(1, col.axis  = "black",at  = seq(60,80,by   =  10),labels  = seq(60,80,by  = 10), las  = 2)
axis(2, col.axis  = "black", at  = seq(20,40,by = 5),labels  = seq(20,40,by  = 5),las  = 0)
title(main = 2014, line = 0.5,cex.main  = 1.5)
mtext("Latitude", side = 2, line = 2,at=30,cex = 1)
mtext("Longitude", side = 1, line = 2,at = 70,cex = 1)
legend("bottomright",title  = "",inset  = .005,bty  = "n",
       c("hotspot","diffusion","dissipation","no process"),fill  = c(colhot,coldif,coldis,colnull),
       density  = c(10,NA,NA,NA),angle  = c(45,NA,NA,NA,NA))
```
